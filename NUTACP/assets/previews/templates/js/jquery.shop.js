$(function() {
    ...
    $(document).on('click', '.badge.clipboard', function() {
        ...
        $('.cacheClp > p').text('Código de la placa ' + code + ' copiado');
        setTimeout(function() {
    ...
    function jetonsMoins(jetons) {
        var jetonsss = $('span.citycash').text().replace(' Diamantes', ''),
    ...
    $(document).on('submit', 'form[name="vip"]', function() {
        ...
        submit.val('Cargando...').attr("disabled", true);
    ...
    $(document).on('submit', 'form[name="sendRoom"]', function() {
        var tthis = $(this),
            ...
            submitVal = submit.val(),
            errorBloc = $('.error');
        errorBloc.text('Cargando...').addClass('success').fadeIn(0);
        submit.val('Cargando...').attr("disabled", true);
        $.post(tthis.attr('action'), {
            username: username,
            roomid: roomid,
