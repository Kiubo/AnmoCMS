...
{
$getRanks = $dbh->prepare("SELECT * FROM permissions WHERE id = 11 ORDER BY id DESC");
}
else
{
$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM ranks WHERE id = 11 ORDER BY id DESC");
}
$getRanks->execute();
...
<h6 class="text-nowrap">Contratación, despido y organización del equipo.</h6>
</label>
</div>
...
if ($config['hotelEmu'] == 'arcturus')
{
$getRanks = $dbh->prepare("SELECT * FROM permissions WHERE id = 9 ORDER BY id DESC");
}
else
{
$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM ranks WHERE id = 9 ORDER BY id DESC");
}
$getRanks->execute();
while ($Ranks = $getRanks->fetch())
{
...
<div class="general-box-header-3 flex">
<div class="general-box-header-3-icon">
<icon name="badge-adm" class="flex margin-auto"></icon>
</div>
<label class="gray flex-column text-nowrap">
<h4 class="bold text-nowrap"><?=$rankName;?></h4>
<h6 class="text-nowrap">Contratación, despido y organización del equipo.</h6>
...
if ($config['hotelEmu'] == 'arcturus')
{
$getRanks = $dbh->prepare("SELECT * FROM permissions WHERE id = 10 ORDER BY id DESC");
}
else
{
$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM ranks WHERE id = 10 ORDER BY id DESC");
}
$getRanks->execute();
while ($Ranks = $getRanks->fetch())
{
...
<div class="general-box-header-3 flex">
<div class="general-box-header-3-icon">
<icon name="badge-ger" class="flex margin-auto"></icon>
</div>
<label class="gray flex-column text-nowrap">
<h4 class="bold text-nowrap"><?=$rankName;?></h4>
<h6 class="text-nowrap">Contratación, despido y organización del equipo.</h6>
...
if ($config['hotelEmu'] == 'arcturus')
{
$getRanks = $dbh->prepare("SELECT * FROM permissions WHERE id = 8 ORDER BY id DESC");
}
else
{
$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM ranks WHERE id = 8 ORDER BY id DESC");
}
$getRanks->execute();
while ($Ranks = $getRanks->fetch())
{
...
<div class="general-box-header-3 flex">
<div class="general-box-header-3-icon">
<icon name="badge-mod" class="flex margin-auto"></icon>
</div>
<label class="gray flex-column text-nowrap">
<h4 class="bold text-nowrap"><?=$rankName;?></h4>
<h6 class="text-nowrap">Modera & organiza eventos para brindar diversión.</h6>
</label>
</div>
<div class="general-box-content staff flex-column bg-2">
<?php
...
if ($config['hotelEmu'] == 'arcturus')
{
$getRanks = $dbh->prepare("SELECT * FROM permissions WHERE id = 7 ORDER BY id DESC");
}
else
{
$getRanks = $dbh->prepare("SELECT id,name,badgeid FROM ranks WHERE id = 7 ORDER BY id DESC");
}
$getRanks->execute();
while ($Ranks = $getRanks->fetch())
{
...
<div class="general-box-header-3 flex">
<div class="general-box-header-3-icon">
<icon name="badge-dev" class="flex margin-auto"></icon>
</div>
<label class="gray flex-column text-nowrap">
<h4 class="bold text-nowrap"><?=$rankName;?></h4>
<h6 class="text-nowrap">Auxiliar de los Moderadores.</h6>
</label>
</div>
<div class="general-box-content staff flex-column bg-2">
<?php
...
El equipo está siempre a su disposición para anticipar todas las necesidades posibles de todos los usuarios y siempre escuchar a la comunidad al máximo.</h5>
<h5 class="margin-bottom-min">Para la seguridad de todos, y especialmente para evitar errores dentro del hotel, todo el personal tiene una tarjeta de identificación que se puede ver claramente al ingresar al hotel.</h5>
<h5 class="margin-bottom-md">
Entonces, si alguien se hace pasar por un miembro del equipo, ¡repórtelo lo antes posible!</h5>
