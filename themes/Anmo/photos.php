...
<link rel="stylesheet" href="<?= $config['hotelUrl'] ?>/<?php echo H. $config['skin']; ?>/cdn/assets/css/photos.css" type="text/css">
...
$userid = $fila['user_id'];
$roomid = $fila['room_id'];
$datos = "SELECT * FROM users WHERE id = $userid";
if($resultado2 = $mysqli->query($datos)) {
while ($dato = $resultado2->fetch_assoc()) {
$datos2 = "SELECT * FROM rooms WHERE id = $roomid";
if($resultado3 = $mysqli->query($datos2)) {
while ($dato2 = $resultado3->fetch_assoc()) {
echo'
<div class="photo-container2">
<div class="photo-content2">
<a href="'.$fila['url'].'" class="photo-picture" target="_blank" style="background-image: url('.$fila['url'].');" data-id="2" data-title="16-Apr 05:34 by jolinangthecute"></a>
<a class="photo-meta flex-container flex-vertical-center">
<div class="photo-meta-left-side"><img src="http://localhost/avatar/avatarimage.php?figure='.$dato['look'].'&gesture=sml&headonly=1&head_direction=2" alt="'.$dato['username'].'" class="pixelated"></div>
<div class="photo-meta-right-side">
<div class="creator-name">'.$dato['username'].'</div>
<div class="published-date">'.$dato2['name'].'</div>
</div>	
</a>
</div>
</div>
